...
function renderEvent(targetMonth) {
  // console.log(targetMonth) //201705
  targetMonth = moment(targetMonth, 'YYYYMM');
  // this.data[targetMonth.get('year')][targetMonth.get('month')]
  let thisDataArr = this.data[targetMonth.get('year')][targetMonth.get('month')].length;
  for (var i = 0; i < thisDataArr; i++) {
    let hasDataDate = this.data[targetMonth.get('year')][targetMonth.get('month')][i].date;
    console.log(hasDataDate)
  }
  ...
  for (let i = 0; i < 42; i++) {
    (function (i) {
      ...
      if (i >= firstWeekDay && i <= monthlyDays + firstWeekDay - 1) { //直到需要加日期那一天
        _date
          .children('span')
